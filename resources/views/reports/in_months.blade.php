...
@if (request('action') && request('book_id') && request('nonce'))
    <div class="card">
        ...
        <div class="card-body">
            ...
            {{ link_to_route('reports.in_months', __('app.cancel'), [], ['class' => 'btn btn-secondary btn-sm']) }}
            {!! Form::submit(__('book.reset_report_title'), ['class' => 'btn btn-secondary btn-sm', 'name' => 'reset_report_title[in_months]']) !!}
        </div>
        {{ Form::close() }}
    </div>
@endif
